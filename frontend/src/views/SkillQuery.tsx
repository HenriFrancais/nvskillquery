...
export function SkillQuery() {
  ...
  const run = useCallback(async () => {
    ...
    setQueryError(null)
    try {
      const res = await api.query(wire, includeNonMatching)
      setResult(res)
      window.history.replaceState(null, '', `?q=${encodeQuery(wire)}`)
    } catch (e) {
      setResult(null)
  ...
  const copyLink = async () => {
    if (!wire) return
    const url = `${window.location.origin}${window.location.pathname}?q=${encodeQuery(wire)}`
  ...
  return (
    <div className="skill-query">
      <div className="page-header">
        ...
        <span className="dim">
          snapshot v{catalog.snapshot_version} ·{' '}
      ...
      <div className="run-bar">
        <button
          ...
          disabled={!wire || running || capWarning !== null}
          onClick={() => void run()}
        >
        ...
        {wire && (
          <a
            className="btn"
            href={api.exportCsvUrl(encodeQuery(wire), includeNonMatching)}
              ...
              {result.rows.map((row) => (
                <tr key={row.user_id} className={row.match_count === 0 ? 'zero-match' : ''}>
                  ...
                  <td className={row.main_character.matches ? 'main-match' : 'main-nomatch'}>
                    ...
                    <span className="dim">({row.main_character.character_type})</span>
                  </td>
                  <td>
                    {row.matching_characters.map((c) => (
                      <span key={c.character_id} className="char-chip">
                        {c.name} <span className="type">{c.character_type}</span>
                      </span>
                    ))}
                  </td>
                  <td className="num">
