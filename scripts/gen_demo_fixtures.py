...
# Demo doctrine fits, shaped like the real doctrine_definitions API: identified
# by (doctrine, role, ship_type, fit_name) and carrying yellow/green skill
# requirements. Skills are sampled from the demo catalogue so they resolve.
DEMO_DOCTRINES = [
    ("Armor Battleships", "DPS", "Megathron", "NV Megathron"),
    ("Armor Battleships", "DPS", "Hyperion", "NV Hyperion"),
    ("Armor Battleships", "Logi", "Guardian", "NV Guardian"),
    ("Shield Cruisers", "DPS", "Caracal", "Standard"),
    ("Shield Cruisers", "DPS", "Caracal", "Kite"),
    ("Shield Cruisers", "Tackle", "Stiletto", ""),  # empty fit_name
    ("Strat T3", "DPS", "Tengu", "HML Tengu"),
    ("Strat T3", "Logi", "Tengu", "Logi Tengu"),
]
N_DOCTRINE_SKILLS = (25, 45)  # inclusive range of skills per fit
...
def _gen_doctrines(rng: random.Random, all_skill_ids: list[int]) -> list[dict]:
    """One fit per DEMO_DOCTRINES entry, each requiring a random sample of
    catalogue skills with a yellow/green level pair (yellow is sometimes 0 =
    not required at the yellow tier, mirroring the real API)."""
    fits = []
    for doctrine, role, ship_type, fit_name in DEMO_DOCTRINES:
        n = rng.randint(*N_DOCTRINE_SKILLS)
        picked = sorted(rng.sample(all_skill_ids, k=min(n, len(all_skill_ids))))
        skills = []
        for sid in picked:
            green = rng.choices([3, 4, 5], weights=[30, 40, 30])[0]
            # ~25% of skills are green-only (yellow 0); otherwise yellow is
            # green or one below, never above green.
            yellow = rng.choices([0, green - 1, green], weights=[25, 35, 40])[0]
            skills.append(
                {"skill_id": sid, "level_yellow": max(0, yellow), "level_green": green}
            )
        fits.append(
            {
                "doctrine": doctrine,
                ...
                "ship_type": ship_type,
                "fit_name": fit_name,
                "fit_eft": f"[{ship_type}, {fit_name or ship_type}]",
                "defining_items": [],
                # Demo keeps it to a single resolved pack; the app flattens
                # across packs regardless.
                "skillpacks": {"[Autogenerated Required Skills]": skills},
            }
        )
    return fits


def generate() -> tuple[dict, dict, dict, list]:
    ...
    # Doctrines reference the same sampled catalogue so they resolve in demo.
    doctrines_payload = _gen_doctrines(rng, all_skill_ids)
    return sde_payload, skills_payload, users_payload, doctrines_payload


def main() -> None:
    sde_payload, skills_payload, users_payload, doctrines_payload = generate()
    ...
    (OUT_DIR / "doctrine_definitions_api.json").write_text(
        json.dumps(doctrines_payload, indent=1) + "\n"
    )
    ...
    print(
        ...
        f"{len(doctrines_payload)} doctrine fits"
    )
